...
export default function Map({
  ...
  useEffect(() => {
    ...
    // Draw control setup (try loading leaflet-draw)
    try {
      if (window.L && window.L.Control && window.L.Control.Draw) {
        ...
        const drawControl = new window.L.Control.Draw({
          ...
          draw: {
            polygon: {
              shapeOptions: { color: '#3b82f6', fillOpacity: 0.15 },
            },
            rectangle: {
              shapeOptions: { color: '#3b82f6', fillOpacity: 0.15 },
            },
      ...
      // Draw control not available — non-critical
    }
        ...
        L.geoJSON(geojson, {
          style: (feature) => {
            if (layerName === 'demographic') {
              ...
              // Green (high density) to red (low density)
            ...
            if (layerName === 'land_use') {
              ...
              return {
                fillColor: LAYER_COLORS.land_use[zone] || '#64748b',
                weight: 0.5,
                opacity: 0.4,
                color: '#0f172a',
                fillOpacity: 0.32,
            ...
            if (layerName === 'environmental') {
              ...
              return {
                ...
                color: c,
                fillOpacity: 0.30,
                dashArray: riskType.includes('flood') ? '4,3' : null,
              }
            }
            if (layerName === 'roads') {
              const roadType = feature.properties.road_type
              return {
                color: LAYER_COLORS.roads[roadType] || '#94a3b8',
                weight: roadType === 'highway' ? 3 : roadType === 'arterial' ? 1.5 : 0.8,
                opacity: roadType === 'highway' ? 0.9 : 0.7,
              }
            }
            return {
              color: '#3b82f6',
          ...
          pointToLayer: (feature, latlng) => {
            if (layerName === 'poi') {
              ...
              return L.circleMarker(latlng, {
                radius: 5,
                fillColor: LAYER_COLORS.poi[cat] || '#94a3b8',
                color: '#0f172a',
                weight: 1,
                fillOpacity: 0.85,
              })
          ...
          onEachFeature: (feature, layer) => {
            ...
            const rows = entries
              .map(([k, v]) => {
                ...
                return `<tr>
                  <td style="color:#94a3b8;padding-right:8px;">${k.replace(/_/g, ' ')}</td>
                  <td style="color:#e2e8f0;font-weight:500;">${val}</td>
                </tr>`
              })
              .join('')
            layer.bindTooltip(
              `<table style="font-size:11px;font-family:system-ui;">${rows}</table>`,
  ...
  useEffect(() => {
    ...
    pinnedSites.forEach((site, idx) => {
      ...
      marker.bindTooltip(
        `Site #${idx + 1} — Score: ${site.composite_score}/100 (${site.grade})`
      )
      marker.addTo(map)
  ...
  useEffect(() => {
    ...
    hotspotData.h3_cells.forEach((cell) => {
      ...
      // Draw approximate H3 hexagon (regular hex, adjusted for lat/lng aspect ratio)
      const r = 0.0028
      ...
      L.polygon(hexPoints, {
        ...
        .bindTooltip(
          `Score: <strong>${cell.score.toFixed(1)}</strong> | ` +
            `Cluster: ${cell.cluster_id >= 0 ? `#${cell.cluster_id}` : 'isolated'}`,
  ...
  useEffect(() => {
    ...
    const group = L.layerGroup()
    const COLORS = ['#8b5cf6', '#3b82f6', '#06b6d4'] // 10min, 20min, 30min

    // Draw largest ring first (so smaller rings are on top)
    ;[...isochroneData.isochrones].reverse().forEach((iso, idx) => {
      ...
      L.geoJSON(
        ...
        .bindTooltip(
          `<strong>${iso.minutes} min ${iso.mode}</strong><br>` +
            `Population: ~${iso.population_within.toLocaleString()}<br>` +
            `Area: ${iso.area_sqkm.toFixed(1)} km²`,
          { className: 'custom-tooltip', sticky: true }
        )
        .addTo(group)
    })
